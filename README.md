...
* **Zero-Data-Loss Incremental Syncing:** The macro remembers the exact timestamp of when your extraction started. On subsequent runs, it incrementally syncs attachments by only extracting files newer than that cutoff date. (Note: To maintain perfect mathematical file-size chunking, email body text is always given a high-speed 100% fresh rewrite, previously extracted attachments are skipped to save processing time).
...
## 🐍 Phase 2: The AI Text Refinery (Python Companion Script)

If you are extracting emails specifically to feed them into a Large Language Model (like Google's NotebookLM, ChatGPT, or Claude), raw email data is often too noisy and fragmented for the AI to understand. 

The **`Create_Reverse_Archive_v2.py`** script acts as a powerful post-processing refinery. It takes the folder-based text chunks generated by the VBA macro and transforms them into a pristine, AI-ready dataset.

### ✨ What It Does
* **Erases AI Hallucination Triggers:** Aggressively strips out all HTTP/HTTPS links, hidden tracking pixels, and HTML artifacts that destroy an LLM's attention mechanism.
* **Global Chronological Sorting:** Ignores your Outlook folder hierarchy and sorts every single email into one continuous, perfect **Reverse-Chronological timeline** (Newest to Oldest). This allows the AI to correctly map cause-and-effect across decades of history.
* **Smart Chunking:** Bypasses strict AI upload limits by automatically repacking the cleaned data into mathematically perfect file sizes (e.g., 3.0 MB).
* **Fixes Invisible Formatting:** Purges zero-width characters, normalizes Windows line breaks, and enforces a strict UTF-8 BOM signature for seamless Google uploads.

### 🚀 How to Use It
1. Ensure you have [Python 3.x](https://www.python.org/downloads/) installed on your machine.
2. Run your VBA `OutlookMiner` macro to extract your `.pst` data.
3. Run the `Create_Reverse_Archive_v2.py` script.
4. A popup will ask you for your target MB chunk size (default is 3.0 MB).
5. Select the **Input Folder** (the `Emails` directory generated by the VBA macro).
6. Select your **Output Destination**.
7. Let the script process! When finished, simply drag and drop the newly generated `Reverse_Master` text files directly into NotebookLM.

### 🛠️ Custom Filters (Advanced)
Every user's email archive has different "noise." If you want to purge massive boilerplate footers (like retail *Unsubscribe* blocks or corporate legal disclaimers), open the Python script in a text editor and look for the `OPTIONAL CUSTOM FILTERS` section. You can easily uncomment the examples or add your own Regex strings to shrink your dataset even further.
